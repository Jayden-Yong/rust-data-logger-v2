...
impl ThingsBoardClient {
    ...
    // pub async fn delete_device(&self, device_id: &str) -> Result<(), TbError> {
    //     let auth_header = self.get_auth_header()?;

    //     let response = self
    //         .client
    //         .delete(&format!("{}/api/device/{}", self.base_url, device_id))
    //         .header("Authorization", auth_header)
    //         .send()
    //         .await?;
    ...
    //     } else {
    //         let error_text = response.text().await.unwrap_or_else(|_| "Unknown error".to_string());
    //         Err(TbError::Api(format!("Delete device failed: {}", error_text)))
    //     }
    // }

    // Send telemetry using device token (for devices to send their own data)
    // pub async fn send_device_telemetry(
    //     &self,
    //     device_token: &str,
    //     telemetry: &HashMap<String, serde_json::Value>,
    // ) -> Result<(), TbError> {
    //     let response = self
    //         .client
    //         .post(&format!("{}/api/v1/{}/telemetry", self.base_url, device_token))
    //         .json(telemetry)
    //         .send()
    //         .await?;
    ...
    //     } else {
    //         let error_text = response.text().await.unwrap_or_else(|_| "Unknown error".to_string());
    //         Err(TbError::Api(format!("Send device telemetry failed: {}", error_text)))
    //     }
    // }
...
// #[tokio::main]
// async fn main() -> Result<(), TbError> {
//     // Example usage
//     let mut client = ThingsBoardClient::new("http://localhost:8080");
    
//     // Login
//     client.login("tenant@thingsboard.org", "tenant").await?;
    
//     // Create a device
//     let new_device = Device {
//         id: None,
//         name: "My Rust Device".to_string(),
//         label: Some("Rust Test Device".to_string()),
//         device_type: "sensor".to_string(),
//         device_profile_id: None,
//     };
    
//     let created_device = client.create_device(&new_device).await?;
//     println!("Created device: {:?}", created_device);
    
//     if let Some(device_id) = &created_device.id {
//         // Get device credentials
//         let credentials = client.get_device_credentials(&device_id.id).await?;
//         println!("Device credentials: {:?}", credentials);
        
//         // Send telemetry data using admin API
//         let mut telemetry = HashMap::new();
//         telemetry.insert("temperature".to_string(), serde_json::json!(25.5));
//         telemetry.insert("humidity".to_string(), serde_json::json!(60.0));
        
//         client.save_device_telemetry(&device_id.id, &telemetry).await?;
//         println!("Telemetry saved successfully");
        
//         // Send telemetry using device token (simulating device sending data)
//         client.send_device_telemetry(&credentials.credentials_id, &telemetry).await?;
//         println!("Device telemetry sent successfully");
//     }
